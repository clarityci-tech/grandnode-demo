...
using System;
namespace Payments.SportsNext.Direct.Validators
{
    public class PaymentInfoValidator : AbstractValidator<PaymentInfoModel>
    {
        public PaymentInfoValidator(SportsNextPaymentsDirectSettings settings, ITranslationService translationService)
        {
            if (settings.Use3DS)
                return;
